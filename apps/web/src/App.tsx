...
const normalizeDraftState = (incoming: DraftState): DraftState => {
  ...
  return {
    ...
    seriesHistory: Array.isArray(incoming.seriesHistory) ? incoming.seriesHistory : [],
...
const GameHistoryCard = ({ game, state }: { game: GameResultSnapshot; state: DraftState }) => {
  ...
  const blueBans = game.blueBans ?? [];
  const redBans = game.redBans ?? [];
  const bluePicks = game.bluePicks ?? [];
  const redPicks = game.redPicks ?? [];
  ...
  return (
    <div className="bg-slate-900/65 backdrop-blur border border-slate-700/60 rounded-2xl p-4 mb-3 shadow-lg shadow-black/20">
      ...
      <div className="flex items-center gap-4 mb-2">
        ...
        <div className="flex gap-1">{blueBans.map((h, i) => <React.Fragment key={i}>{renderMini(h, 'BAN')}</React.Fragment>)}</div>
        <div className="flex gap-1 ml-auto">{bluePicks.map((h, i) => <React.Fragment key={i}>{renderMini(h, 'PICK')}</React.Fragment>)}</div>
      </div>

      <div className="flex items-center gap-4">
        <div className="w-28 text-right text-red-300 font-black text-xs">{redTeamName}</div>
        <div className="flex gap-1">{redBans.map((h, i) => <React.Fragment key={i}>{renderMini(h, 'BAN')}</React.Fragment>)}</div>
        <div className="flex gap-1 ml-auto">{redPicks.map((h, i) => <React.Fragment key={i}>{renderMini(h, 'PICK')}</React.Fragment>)}</div>
      </div>
    </div>
  );
};
